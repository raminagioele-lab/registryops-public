# RegistryOps V1 Event Types

This document defines the closed set of authorized `event_type` values for RegistryOps V1.
...
RegistryOps V1 authorizes only a minimal closed vocabulary of event types.
...
No `event_type` outside this document is allowed in RegistryOps V1.
...
No other `event_type` value is allowed in RegistryOps V1.
...
The `event_type` vocabulary of RegistryOps V1 is closed.
...
Any extension of the `event_type` vocabulary requires an explicit revision of the canonical specification and therefore belongs to a future version, not to V1.

## Compliance Rule

An implementation is compliant with RegistryOps V1 only if it emits events using exclusively the seven authorized `event_type` values defined in this document and in the canonical specification.