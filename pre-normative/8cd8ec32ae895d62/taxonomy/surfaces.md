# RegistryOps Surface Types

This document defines the closed set of authorized `surface_type` values for RegistryOps.
...
RegistryOps authorizes only a minimal closed vocabulary of surface types.
...
No `surface_type` outside this document is allowed in RegistryOps.
...
The `surface_type` vocabulary of RegistryOps is closed.
...
Any extension of the `surface_type` vocabulary requires an explicit revision of the canonical specification and therefore belongs to a future version, not to the current canonical taxonomy.

## Compliance Rule

An implementation is compliant with RegistryOps only if it emits events using exclusively the seven authorized `surface_type` values defined in this document and in the canonical specification.